...
import 'package:sdgp_test01/presentation/frame_981_screen/frame_981_screen.dart';

class Frame926Page extends StatefulWidget {
  const Frame926Page({Key? key}) : super(key: key);

  @override
  Frame926PageState createState() => Frame926PageState();
}

class Frame926PageState extends State<Frame926Page> {
  ...
  String topClothingImageUrl = ''; // Initially empty, to be fetched from backend
  String bottomClothingImageUrl = ''; // Initially empty, to be fetched from backend
  ...
  void _scrollToCurrentDate() {
    ...
    double offset = (currentDayIndex * 64.h) +
        (currentDayIndex * 15.0); // Calculate offset
  ...
  void _fetchOutfitData() async {
    ...
    setState(() {
      topClothingImageUrl = 'assets/images/img_filled_appointment_reminders.png';
      bottomClothingImageUrl = 'assets/images/img_filled_appointment_reminders.png';
    ...
    return Scaffold(
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            SizedBox(height: 46), // Consider adjusting this height if it's too large
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: weekDays.asMap().entries.map((entry) {
          ...
          DateTime dayDate = specificCurrentDate.add(Duration(days: index - specificCurrentDate.weekday + 1));
          return Padding(
            padding: EdgeInsets.only(right: index < weekDays.length - 1 ? 15.0 : 0.0),
            child: Column(children: [
              CustomTextFormField(
                width: 64.h,
                hintText: day, // Day of the week
                textInputAction: TextInputAction.done,
                contentPadding: EdgeInsets.symmetric(horizontal: 19.h, vertical: 4.v),
                borderDecoration: TextFormFieldStyleHelper.fillGray,
                fillColor: appTheme.gray60002,
              ),
              Container(
                width: 64.h,
                padding: EdgeInsets.symmetric(horizontal: 0.h, vertical: 9.v),
                decoration: AppDecoration.fillBlueGray.copyWith(borderRadius: BorderRadiusStyle.customBorderBL20),
                alignment: Alignment.center,  // Aligns the child to the center of the container
                child: Text(
                  dayDate.isAtSameMomentAs(specificCurrentDate) ? "Today" : dayDate.day.toString(),
                  style: theme.textTheme.bodyLarge,
                  textAlign: TextAlign.center,  // Centers the text inside the Text widget
                ),
              ),
            ]),
          );
    ...
    showDialog(
      ...
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          content: Text('Do you want to keep or stop syncing calendar events from this device?'),
          actions: <Widget>[
            TextButton(
              onPressed: () {
                Navigator.pop(dialogContext);
                setState(() {
                  shouldShowSyncSection = false; // Do not sync and hide the section
                });
              },
  ...
  Widget _buildSyncCalendarEventsSection() {
    ...
    return Container(
      ...
      decoration: AppDecoration.fillBlueGray.copyWith(borderRadius: BorderRadiusStyle.roundedBorder12),
      child: Row(
        ...
        children: [
          ...
          Padding(
              ...
              child: Text("Sync calendar events from device?", style: theme.textTheme.bodySmall)
          ),
          SizedBox(width: 10),
          GestureDetector(
            onTap: ()  {
            ...
            child: Image.asset(
              ImageConstant.redcross, // Replace with your red cross icon asset path
              height: 10.v,
              width: 10.h,
            ),
          ),
          SizedBox(width: 10),
          _buildGoogleSignIn(context), // This replaces the previous CustomImageView for the green tick icon
        ],
      ),
    );
  }
...
// And here is the _buildGoogleSignIn method as before
      ...
      onTap: () async {
        try {
          ...
          if (account != null) {
            ...
            final cal.Events events = await calendarApi.events.list("primary");
            setState(() {
              _calendarEvents = events.items ?? []; // Provide an empty list as default value
            });
          }
        } catch (error) {
          print("Error syncing calendar: $error");
  ...
  Widget _buildOutfitSection(String topClothingImageUrl, String bottomClothingImageUrl, String outfitNumber) {
    ...
    Widget topClothingImageWidget = isTopImageUrlValid
        ...
        : Image.asset('assets/images/img_filled_appointment_reminders.png', fit: BoxFit.cover); // Using a local asset as a placeholder
    ...
    String displayOutfitNumber = outfitNumber.isNotEmpty ? outfitNumber : "Loading outfit...";
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Container(
                ...
                child: Text(currentDate, style: theme.textTheme.bodyMedium),
              ),
              Container(
                decoration: BoxDecoration(
                  ...
                  child: GestureDetector(
                    onTap: () {
                      Navigator.of(context).push(
                        MaterialPageRoute(builder: (context) => Frame981Screen()),
                      );
                    },
                    child: Padding(
                      padding: EdgeInsets.all(4.0), // Optional padding for the SVG
                      child: SvgPicture.asset(
                        'assets/images/img_calendar.svg', // Adjust asset path if necessary
          ...
          Align(
            alignment: Alignment.centerRight, // This aligns the container to the right
            child: Container(
              ...
              child: Padding(
                ...
                child: Center( // Center the text within the container
                  child: Text(displayOutfitNumber, style: theme.textTheme.bodyLarge),
                ),
              ),
            ),
          ),
  ...
  Widget _buildBottomContainer() {
    return Container(
      width: 360, // Fixed width set to 250
      height: 320, // Fixed height set to 300// Takes the full width of the screen
      padding: EdgeInsets.symmetric(horizontal: 16, vertical: 20), // Outer padding
      decoration: BoxDecoration(
      ...
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        crossAxisAlignment: CrossAxisAlignment.center, // Aligns children to center horizontally
        children: [
          _buildInfoContainer('Outfit for weather', 18), // Info container for 'Outfit for weather'
          SizedBox(height: 10),
          _buildPlaceholderIcon(), // Placeholder for the image
          SizedBox(height: 10),
          _buildInfoContainer('Recommend:', 14),
          SizedBox(height: 10),
          _buildInfoContainer('Weather is Cold, Wear a Longsleeve', 14), // Info container for recommendation
        ],
      ),
    );
  }
...
List<String> _getCurrentWeekDays() {
  ...
  return List.generate(7, (index) => DateFormat('EEE').format(monday.add(Duration(days: index))));
}
  ...
  onTapBtnCalendar(BuildContext context) {
    Navigator.pushNamed(context, AppRoutes.frame981Screen);
  }
